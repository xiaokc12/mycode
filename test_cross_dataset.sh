...
for cls_id in "${!cls_ids[@]}";do
    ...
    for i in "${!depth[@]}";do
        ...
        ## train on the VisA dataset
            ...
            --data_path ./data/Eyecandies_processed --save_path ./results/mvtec_${base_dir}_pc_336_4_all/with_color_max_scor${obj_list[cls_id]}_transfer/zero_shot \
...
for cls_id in "${!cls_ids[@]}";do
    ...
    for i in "${!depth[@]}";do
        ...
        ## train on the VisA dataset
            ...
            --data_path ./data/Real3D-AD --save_path ./results/mvtec_${base_dir}_pc_336_4_all/with_color_max_scor${obj_list[cls_id]}_transfer/zero_shot \
            --checkpoint_path ${save_dir}epoch_15.pth \
            --features_list 6 12 18 24 --image_size 336 --depth ${depth[i]} --n_ctx ${n_ctx[j]} --t_n_ctx ${t_n_ctx[0]} --train_class ${obj_list[cls_id]}
        wait
        done
